...
const KATA_KASAR_BASE = [
    'anjing', 'anjir', 'asu', 'bangsat', 'babi', 'kampret', 
    'tolol', 'goblok', 'idiot', 'bodoh', 'tai', 'ngentot',
    'memek', 'kontol', 'jancok', 'cok', 'bajingan', 'monyet',
    'brengsek', 'sialan', 'setan', 'iblis', 'pantek', 'puki',
    'anjg', 'bgst', 'tlol', 'gblk', 'njir', 'b4bi', 'k0nt0l'
];

// Keyword untuk deteksi ajakan main ML
const ML_KEYWORDS = [
    'main ml', 'mabar ml', 'ml yuk', 'mobile legend', 'mobile legends',
    'push rank', 'classic ml', 'ranked ml', 'ml bareng', 'mabar mobile',
    'ada yang ml', 'ml gak', 'ml ga', 'yuk ml', 'gas ml',
    'ml dulu', 'ngajakin ml', 'ajak ml', 'main mole', 'mabar mole'
];

const SUSPICIOUS_KEYWORDS = [
    'klik link', 'menang hadiah', 'gratis', 'promo', 'diskon 90%',
    'jangan lewatkan', 'buruan', 'terbatas', 'claim sekarang',
    'transfer sekarang', 'investasi cuan', 'passive income'
];
...
function analyzeMessageContext(message, userHistory) {
    ...
    const hasAggressiveWord = aggressiveWords.some(w => message.body.toLowerCase().includes(w));
...
function detectKataKasar(message) {
    ...
    // Gabungkan skor
    const finalConfidence = Math.min((totalConfidence + contextAnalysis.toxicScore) / 2, 1);
    ...
    return {
        ...
        severity: finalConfidence > 0.8 ? 'high' : finalConfidence > 0.5 ? 'medium' : 'low'
    };
}
...
function detectSpamRuleBased(message) {
    ...
    const keywordMatches = SUSPICIOUS_KEYWORDS.filter(kw => text.includes(kw.toLowerCase()));
...
client.on('message_create', async (message) => {
    try {
        ...
        if (toxicDetection.isToxic) {
            ...
            console.log('Confidence:', (toxicDetection.confidence * 100).toFixed(1) + '%');
            ...
            if (toxicDetection.violations.length > 0) {
                const detected = toxicDetection.violations.map(v => {
                    const typeEmoji = {
                        'exact': '🎯',
                        'similar': '🔄',
                        'contains': '📍',
                    ...
                    return `${typeEmoji[v.type] || '⚠️'} "${v.original}" (${(v.confidence * 100).toFixed(0)}% mirip "${v.matched}")`;
                }).join('\n');
                detailMsg = `\n\n*Terdeteksi:*\n${detected}`;
            }
            ...
            if (userData.count === 1) {
                warningMsg = `⚠️ *PERINGATAN 1/3*\n\n@${message.author.split('@')[0]}, mohon jaga bahasa di grup ini ya! 🙏${detailMsg}\n\n_Confidence: ${(toxicDetection.confidence * 100).toFixed(0)}%_`;
            } else if (userData.count === 2) {
                warningMsg = `⚠️ *PERINGATAN 2/3*\n\n@${message.author.split('@')[0]}, ini peringatan kedua! Harap gunakan bahasa yang sopan. ⚠️${detailMsg}`;
            } else if (userData.count >= 3) {
                warningMsg = `🚨 *PERINGATAN TERAKHIR (3/3)*\n\n@${message.author.split('@')[0]}, pelanggaran berulang terdeteksi!\n\nAdmin akan meninjau akun Anda. Pelanggaran selanjutnya dapat berakibat kick dari grup.${detailMsg}`;
            }
        ...
        if (mlDetection.isMLInvitation) {
            ...
            const mentions = participants.filter(p => p !== client.info.wid._serialized);

            const contact = await message.getContact();
            const mlMsg = `🎮 *MOBILE LEGENDS PARTY!* 🎮\n\n` +
...
client.on('message', async (message) => {
    ...
    if (text === '!spam-stats') {
        ...
        const statsMsg = '📊 *Spam Statistics*\n\n' +
            (stats.length > 0 
                ? stats.map((s, i) => `${i+1}. @${s.user}: ${s.count} pesan/menit`).join('\n')
    ...
    if (text === '!toxic-stats') {
        ...
        const statsMsg = '🤬 *Toxic User Statistics*\n\n' +
            (stats.length > 0 
                ? stats.map((s, i) => `${i+1}. @${s.user}: ${s.count} peringatan`).join('\n')
    ...
    if (text === '!learned-words') {
        ...
        const suspiciousStats = Array.from(learnedPatterns.suspiciousWords.entries())
        ...
        if (suspiciousStats.length > 0) {
            ...
            msg += suspiciousStats.map(([word, stats]) => 
    ...
    if (text === '!bot-help') {
        const helpMsg = `🤖 *Bot Command List*\n\n` +
            `📊 *Statistik:*\n` +
